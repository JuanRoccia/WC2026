...
for row in ANNEX_C_ROWS:
    key = "".join(sorted(row))
    ANNEX_C_LOOKUP[key] = row


def get_third_place_assignments(advancing_groups: list[str]) -> dict[str, str]:
    key = "".join(sorted(advancing_groups))
    row = ANNEX_C_LOOKUP.get(key)
    if row is None:
        return {}
    return dict(zip(ANNEX_C_WINNERS, row))
